...
int main()
{
	...
	plf::colony<payload> dead_map;

	for (uint64_t index = 0U; index < ELEMENT_COUNT / 10ULL; ++index)
	{
		dead_map.emplace(plf::rand() % 100U, plf::rand() % 100U, plf::rand() % 100U, plf::rand() % 100U);
	}
	...
	plf::colony<payload> map(ELEMENT_COUNT);
	...
	scw::print_micro(map_insert_t, "Colony Allocate");
	scw::print_micro(map_iter_t, "Colony Dense Iterate");
	scw::print_micro(map_erase_t, "Colony Erase");
	scw::print_micro(map_sparse_iter_t, "Colony Sparse Iterate");
	scw::print_micro(map_reinsert_t, "Colony Reinsert");
	scw::print_micro(map_reinsert_iter_t, "Colony Reinsert Iterate");

	printf("\n");

	scw::print(iter_sum, "Colony Sum");
	scw::print(map_sparse_sum, "Colony Sparse Sum");
	scw::print(map_reinsert_iter_sum, "Colony Reinsert Sum");
	scw::print(dead_sum);
